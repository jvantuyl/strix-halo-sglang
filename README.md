...
Plus **patches 11 to 31**, which bring Qwen3.8-Flash-Next (Qwen4-Exp) up on gfx1151:
...
16. **[Dense compressed-tensors int4 Linear on ROCm](patches/16-wna16-rocm-dense.md)** — the dense WNA16 scheme is Marlin-only and Marlin is CUDA-only, so any checkpoint that quantizes attention `q/k/v/o`, shared experts or `lm_head` died with `NameError: gptq_marlin_repack`. Dequantize to bf16 once at load and serve with `F.linear`; unlocks e.g. the abliterated [`davetha/…DERISKED-W4A16-AWQ`](docs/RUNNING_QWEN38.md#running-the-abliterated-variant-derisked) requant. Apply with [`patches/patch_wna16_rocm_dense.py`](patches/patch_wna16_rocm_dense.py).
17. **[`SGLANG_MOE_CONFIG_DIR` as a search path](patches/17-moe-config-dir.md)** — upstream's only knob for external MoE tile configs replaces the builtin tree, requires its `configs/triton_x_y_z/` layout and crashes on a missing directory. It is now a list of directories searched first (flat or tree layout), so tuned tiles are mounted per checkpoint instead of baked into the image. Not gfx1151-specific. Apply with [`patches/patch_moe_config_dir.py`](patches/patch_moe_config_dir.py).
18. **[Deterministic HyperConnection mix on ROCm](patches/18-hc-mix-rocm.md)** — the sm_100 JIT mix is unavailable, so every decode step ran upstream's persistent Triton kernel, whose split-K `atomic_add` made greedy decode differ run to run (different logprobs from the first token, divergent text on near-ties) and whose software grid barrier assumes co-resident CTAs. A two-launch variant (per-split partials, fixed-order reduction, no barrier) is used on HIP: bit-identical across cold runs at the same decode speed, where falling back to the torch path cost 35%. Apply with [`patches/patch_hc_mix_rocm.py`](patches/patch_hc_mix_rocm.py).
19. **[Masked weight load on a partial K block in the GPTQ/AWQ MoE kernel](patches/19-moe-wna16-kmask.md)** — when K is not a multiple of `BLOCK_SIZE_K` the kernel masked the activations and scales but read the packed weights unmasked, past the last expert's rows: a layout-dependent GPU page fault that killed the tile tuner on the g128 checkpoint at its first `BLOCK_SIZE_K=128` config. Same code for even K; not gfx1151-specific. Apply with [`patches/patch_moe_wna16_kmask.py`](patches/patch_moe_wna16_kmask.py).
20. **[Decode QSA block selection honours the HIP top-k guard](patches/20-qsa-decode-topk.md)** — `select_decode_tokens` called the JIT `fast_topk` directly, bypassing patch 11's guard, so every decode step ran the kernel patch 11 found unsafe on RDNA 3.5; its output order also varies run to run past 512 compressed blocks, which left long-context greedy decode non-deterministic after patch 18. A vectorised, graph-capturable torch top-k replaces it on HIP (~1–1.5 ms per decode step across the 12 QSA layers). Apply with [`patches/patch_qsa_decode_topk.py`](patches/patch_qsa_decode_topk.py).
21. **[Tie-stable QSA block selection on ROCm](patches/21-qsa-topk-ties.md)** — `torch.topk` on this ROCm build returns tied entries in a different order per launch (and a different set when the tie sits on the k boundary), and the indexer's `relu` block scores tie constantly; both HIP selection paths ran through it, so prefill above ~1.4k tokens still drifted bit-wise. A stable sort (prefill) and a top-k over unique score+index keys (decode) break ties toward the lower block; the prefill form also replaces a per-row Python loop (119 → 3.5 ms per QSA layer at 1.5k tokens). Apply with [`patches/patch_qsa_topk_ties.py`](patches/patch_qsa_topk_ties.py).
22. **[Greedy first draft under rejection sampling](patches/22-spec-draft-greedy.md)** — HIP defaults EAGLE/NEXTN to rejection sampling, whose per-step proposal gives greedy rows their argmax, but the draft pass after prefill called `fast_sample` directly: the first draft token of every request was a random draw at `temperature=0`, so greedy MTP output differed run to run (through the verify batch shape, not through any emitted sampled token) and gave away accept length. Routes it through the same proposal. Not gfx1151-specific. Apply with [`patches/patch_spec_draft_greedy.py`](patches/patch_spec_draft_greedy.py).
23. **[MTP draft decode sees the drafted tokens](patches/23-qsa-mtp-tail.md)** — the shared MTP block selection appended the positions drafted since the capture after the captured row's `-1` padding, but the KV gather packs a row's valid entries as a prefix (a count, not a mask): the drafted positions were dropped and the packed slot they should fill was left unwritten, stale scratch on the FA2 path here (output depended on the previous request) and zeros on the paged path. Places the tail right after the captured entries; accept length rose 2.3 → 2.55 on short prompts. Not gfx1151-specific. Apply with [`patches/patch_qsa_mtp_tail.py`](patches/patch_qsa_mtp_tail.py).
24. **[Length-bounded Triton decode MQA for the QSA indexer](patches/24-qsa-mqa-triton.md)** — without TileLang (not installed on ROCm, nor on every CUDA build) the indexer's decode block scoring ran upstream's torch reference, which gathers the whole `context_length / 4` window per row on every decode step of all 12 QSA layers: 1.8 ms per layer at bs 20 with a 32k context, 6.0 ms at 131k. That was the entire 99 → 71 tok/s drop at 16–20 streams when the launcher's default context grew (bisected by rebuilding the old image and crossing image × context). One Triton launch bounded by each row's length, bit-identical, graph-capturable: 16 / 20 streams back to 89–105 / 95–104 tok/s at the full 131k context, MTP 42 / 47 / 52 → 50 / 60 / 68 at 4 / 8 / 10. `SGLANG_QSA_MQA_TRITON=0` for A/B. Apply with [`patches/patch_qsa_mqa_triton.py`](patches/patch_qsa_mqa_triton.py).
25. **[Row-sliced prefill block selection](patches/25-qsa-topk-slices.md)** — patch 21's stable sort ran over the whole 8192-row prefill chunk, ~1 GiB of transient buffers per QSA layer and 40% of the prefill's VRAM peak on a box that idles at 90 of 96 GiB with MTP and 20 requests. Sort in row slices into a preallocated output: identical indices, ~70 MiB. Apply with [`patches/patch_qsa_topk_slices.py`](patches/patch_qsa_topk_slices.py).
26. **[Triton prefill MQA for the QSA indexer](patches/26-qsa-mqa-prefill-triton.md)** — the prefill twin of patch 24: without TileLang the reference `einsum` materialises the per-head scores (4× the 128 MiB logits budget) plus three copies, ~1.15 GiB per layer per chunk. `tl.dot` kernel writing only the logits, 2.9e-6 from the reference, 5× faster; the prefill transient is now flat in prompt length (~2 GiB of per-chunk activations). Shares patch 24's `SGLANG_QSA_MQA_TRITON=0`. Apply with [`patches/patch_qsa_mqa_prefill_triton.py`](patches/patch_qsa_mqa_prefill_triton.py).
27. **[Bandwidth-free weights in pinned system memory](patches/27-host-parked-params.md)** — the token embedding (a `bs`-row gather per step, 1.18 GiB) and the vision tower (idle without images, 0.9 GiB) hardly get read, yet sit in a carve-out that idles at 90 of 96 GiB with MTP and 20 requests. Exactly sized `hipHostMalloc` buffers through ctypes, aliased as CUDA tensors via `__cuda_array_interface__`, owned by the tensor storage; `SGLANG_HOST_PARKED_PARAMS` name patterns, off when unset. ~2 GiB of VRAM back (worst-case headroom 1.6 → 3.3 GiB) for the same 2 GiB of host RAM; greedy output, vision and throughput unchanged. Apply with [`patches/patch_host_parked_params.py`](patches/patch_host_parked_params.py).
28. **[PLE short conv over the packed prefill batch](patches/28-ple-short-conv-packed.md)** — the PLE layers' depthwise conv padded every prefill request to the longest one in the batch, three copies of `[requests, longest, 10240]`: chunked prefill mixing 17 short prompts with a 6k-token slice of a long one was 6.3 GiB (9.6 at the cap) and 714 ms per layer, found when the benchmark suite's mixed scenario hit 117 MiB of free VRAM. One conv over the packed tokens with each request's state spliced in: bit-identical, 431 MiB, 60× faster. Apply with [`patches/patch_ple_short_conv_packed.py`](patches/patch_ple_short_conv_packed.py).
29. **[A request's `reasoning_effort` beats `--default-chat-template-kwargs`](patches/29-default-effort-override.md)** — the OpenAI endpoint pops a request's `reasoning_effort` out of its `chat_template_kwargs`, `setdefault`s the server defaults into the emptied slot, then merges the kwargs over the request field, so once the server default is `medium` every request renders at `medium` (`prompt_tokens` identical for low / medium / xhigh). Seed the merge with the request's effort first. Not gfx1151-specific; same code on upstream `main`. Apply with [`patches/patch_default_effort_override.py`](patches/patch_default_effort_override.py).
30. **[The MTP draft loads only the shards that hold MTP tensors](patches/30-draft-mtp-shards.md)** — the draft's `load_weights` drops every name without `mtp`, yet its loader walked all 34 shards (222,718 mostly 25 KiB expert tensors plus the 26 PLE shards) to keep 31 tensors from three files: 49 s of every boot. A `weight_files_to_skip` hook on the MTP class, decided from the safetensors index. Apply with [`patches/patch_draft_mtp_shards.py`](patches/patch_draft_mtp_shards.py).
31. **[Presharded dump/reload with a host-resident PLE table](patches/31-presharded-host-tables.md)** — upstream's `--load-format presharded` copies the post-processed state back on later boots instead of walking the checkpoint's 222,718 small tensors (the load is per-tensor overhead, not I/O: 0.68 ms each, disk at 4.2 GB/s). On this model the 47.7 GB file-backed PLE table is a `Parameter` the dump would hash and rewrite, its completion marker is only checked from `load_weights` (never called on reload), and `load_weights` ends with the GDN in_proj fusion. Skip host tensors, check the marker before trusting a dump (fall back to a normal load otherwise), run the fusion after the copy. The key also takes `SGLANG_PRESHARDED_STAMP` (the launcher passes the image ID) so a rebuilt image never reuses a dump its patches did not produce; an interrupted dump of the current key is removed and redone, other subfolders are reported and left alone. `QWEN38_PRESHARDED_DIR` in the launcher. Apply with [`patches/patch_presharded_host_tables.py`](patches/patch_presharded_host_tables.py).

On top of those, [`configs/moe/`](configs/moe/) ships fused-MoE Triton tiles for `E=512,N=320,int4_w4a16` found by [`tools/tune_moe_gfx1151.py`](tools/tune_moe_gfx1151.py) (upstream's tuner with a gfx1151 search space and an early bail-out that cuts the sweep by ~4×): the generic tile was 2.2× slower at decode sizes; end to end +15% single-stream and +35% at 8 streams. The launchers and `compose.yaml` mount one profile directory per checkpoint (`configs/moe/qwen38-flash-next`, `configs/moe/qwen3.5-35b-a3b`) at `/moe-configs` via patch 17; the image itself carries no tuning data.
